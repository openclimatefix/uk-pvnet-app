...
def check_model_nwp_inputs_available(
    ...
    if model_uses_nwp and (nwp_valid_times is None):
        ...
        req_start_time = (t0 - pd.Timedelta(f"{nwp_config.history_minutes}min")).ceil(
            freq,
        )

        req_end_time = (t0 + pd.Timedelta(f"{nwp_config.forecast_minutes}min")).ceil(
            freq,
        )
...
class NWPDownloader(ABC):
    ...
    def run(self) -> None:
        ...
        if self.source_path is None:
            logger.warning(
                f"Source file for {self.nwp_source} is not set. Skipping download.",
            )
            return
...
class ECMWFDownloader(NWPDownloader):
    ...
    def regrid(ds: xr.Dataset) -> xr.Dataset:
        ...
        return regrid_nwp_data(
            ds=ds,
            target_coords_path=files("pvnet_app.data").joinpath(
                "nwp_ecmwf_target_coords.nc",
            ),
    ...
    def extend_to_shetlands(ds: xr.Dataset) -> xr.Dataset:
        ...
        return ds.reindex(
            latitude=np.concatenate([np.arange(62, 60, -0.05), ds.latitude.values]),
        )
